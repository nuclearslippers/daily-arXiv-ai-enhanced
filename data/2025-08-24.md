...
TL;DR: 统一的单阶段同训框架，通过魔法标记动态切换三种安全行为模式，在保持安全性的同时提供精细控制能力


<details>
  <summary>Details</summary>
Motivation: 解决当前LLM内容安全方法依赖多阶段训练流水线、缺乏部署后精细控制能力的问题

Method: 提出统一同训框架，在单个SFT阶段集成三种安全行为：正向（合规/利他）、负向（无过滤/高风险）和拒绝向（保守/拒绝），通过系统指令或魔法标记动态激活

Result: 方法达到SFT+DPO的安全对齐质量，8B模型在安全性能上超过DeepSeek-R1(671B)，同时显著降低训练复杂度和部署成本

Conclusion: 该工作提供了一种可扩展、高效且高度可控的LLM内容安全解决方案，通过安全对齐边际实现了削漏强化和精细控制
...
Result: 生成了基于自动指标的初步排名，但指出该排名可能存在偏向重排序技术的偏差

Conclusion: 自动排名是初步结果，最终WMT25官方排名将基于更可靠的人工评估，该报告主要为参与者提供参考
...
TL;DR: 这篇论文提出了一种LLM驱动的文化本土化框架，用于自动生成包含本土实体的多语言数学应用题数据集，解决低资源语言中的文化基础数学推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有多语言数学评测集主要通过翻译生成，保留了英语中心的实体，导致低资源语言的文化基础数学推理能力落后，需要解决本土化数据集稀缺的问题。

Method: 设计了一种LLM驱动的文化本土化框架，能够从现有数据源自动构建包含本土名字、组织名称和货币等原生实体的数据集。

Result: 实验结果显示，翻译的评测集可能隐藏了真实的多语言数学能力，而本文框架能够减少英语中心实体偏见，提高了各种语言中本土实体出现时的模型稳健性。

Conclusion: 该研究为多语言数学推理提供了有效的文化本土化解决方案，证明了适当社会文化语境下真正多语言数学能力评估的重要性。
...
TL;DR: 通过DPO偏好优化技术，使用程序化筛选的语料对进行精细调整，提升了斯洛文翻译质量，COMET指标显著提高且更稳定避免语言错误。


<details>
  <summary>Details</summary>
Motivation: 大语言模型已成为有效的机器翻译系统，但如何用较少资源细调以专门优化翻译性能仍是挑战。本文以斯洛文为案例，探索如何通过偏好优化技术提升通用指令调整模型的翻译能力。

Method: 使用Direct Preference Optimization (DPO)技术，通过程序化筛选和增强公开数据集子集进行训练。生成训练数据时，使用GaMS-9B-Instruct和EuroLLM-9B-Instruct两个LLM翻译英语维基百科文章，然后基于合成评估指标如COMET的约束来评分翻译质量并生成质量排名的语料对。

Result: 细调后的模型在翻译维基百科文章时，COMET指标相比两个基准模型分别提高了约0.04和0.02。同时更一致地避免了语言和格式错误，显示出更好的翻译质量和稳定性。

Conclusion: 通过程序化筛选质量排名语料对并使用DPO技术进行细调，可以用较少资源显著提升通用指令调整模型的翻译性能，为小语种翻译提供了一种高效的优化方案。
...
Method: 扩展CoXQL数据集为MultiCoXQL（5种语言），提出新的解析方法，创建Compass自定义输入数据集，并使用多种LLM进行评估。

Result: 在MultiCoXQL上评估了三种LLM的解析性能，在Compass上进行了单语、跨语言和多语言评估，比较了不同规模的LLM和BERT类模型。

Conclusion: 提出的多语言数据集和解析方法有助于提升ConvXAI系统的多语言泛化能力和自定义输入处理能力。
...
TL;DR: 提出奖励偏移推测采样算法(SSS)，通过将小型草稿模型与人类偏好对齐，而保持目标模型不变，在降低推理成本的同时实现测试时对齐效果


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐技术虽然能提升LLM安全性和推理能力，但推理成本过高限制了实际应用，需要寻找更高效的解决方案

Method: 采用推测采样加速思想，让对齐后的草稿模型预测未来token，通过修改接受准则和奖励token分布来利用对齐草稿模型与未对齐目标模型之间的分布偏移

Result: 在测试时弱到强对齐实验中，以显著降低的推理成本获得了优越的黄金奖励分数

Conclusion: SSS算法既能有效实现测试时对齐，又能大幅降低推理成本，解决了测试时对齐技术的效率瓶颈问题
...
TL;DR: LongRecall是一个三阶段召回评估框架，通过分解答案、词汇语义过滤和结构化蕴含检查来提升机器生成文本的完整性评估准确性
...
Method: 将LLM与组合推理相结合，探索有效的提示策略来估计LLM置信度，使用符号推理方法，并进行校准和基于结构化预测目标的微调

Result: 无论采用何种提示策略，在提示基础上添加符号推理都能产生更一致和准确的预测，结构化学习能提升挑战性任务的性能

Conclusion: 符号推理能有效弥补LLM的结构一致性不足，结构化学习在大语言模型时代仍然具有重要意义
...
TL;DR: 提出了Nemotron-CC-Math，一个从Common Crawl构建的大规模高质量数学语料库，使用新颖的领域无关管道来可靠提取科学内容，包括数学公式，显著提升LLM的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的数学数据集由于脆弱的提取启发式方法、有损的HTML到文本转换以及无法可靠保留数学结构而导致质量下降，需要更稳健的科学文本提取方法。

Method: 使用布局感知渲染（lynx）和基于LLM的针对性清理阶段的新管道，恢复各种格式的数学内容（MathJax、KaTeX、MathML），保留方程和代码块的结构完整性，同时去除样板文件并将符号标准化为LaTeX表示。
...
Conclusion: 这是第一个从嘈杂的网络规模数据中可靠提取科学内容（包括数学）的管道，在数学、代码和一般推理方面产生可衡量的收益，并设置了开放数学预训练语料库的新技术水平。
...
TL;DR: 该论文提出了一种利用外部证据识别和回答包含错误假设问题的方法，通过事实验证来减少LLM的幻觉问题。
...
Conclusion: 利用外部证据和原子假设验证的方法能有效识别和回答包含错误假设的问题，减少LLM的幻觉问题并提高答案的可解释性。
...
TL;DR: 提出了MMDR-Bench多模态对话推理基准和CoLVLM Agent框架，通过记忆-感知-规划-执行的迭代循环增强现有LVLM模型，在复杂多轮多模态任务上显著超越GPT-4o和Gemini 1.5 Pro等商业模型。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型和大视觉语言模型在处理需要深度推理、持续上下文理解、实体追踪和多步骤指令跟随的复杂多轮视觉基础任务时仍面临重大挑战，现有基准无法捕捉真实世界多模态交互的动态性和复杂性。

Method: 提出MMDR-Bench数据集（300个复杂多轮对话场景）和CoLVLM Agent框架，采用迭代的"记忆-感知-规划-执行"循环机制，无需对底层模型进行大规模重新训练。

Result: 在MMDR-Bench上CoLVLM Agent平均人类评估得分4.03，显著超越GPT-4o（3.92）和Gemini 1.5 Pro（3.85），在推理深度、指令遵循和错误抑制方面表现优异。

Conclusion: 该框架的模块化设计和迭代方法能有效处理复杂多模态交互，在扩展对话轮次中保持稳健性能，验证了其有效性。
...
TL;DR: SemToken是一种语义感知的分词框架，通过语义聚类减少冗余分词，在保持模型性能的同时显著减少token数量和加速计算


<details>
  <summary>Details</summary>
Motivation: 现有分词方法如BPE和WordPiece仅基于频率统计，忽略了文本的语义结构，导致语义冗余区域的过度分词和上下文连贯性利用不足，特别是在长文本场景中

Method: 首先通过轻量级编码器提取上下文语义嵌入，进行局部语义聚类合并语义等价的token；然后基于语义密度分配异构分词粒度，在内容丰富区域使用细粒度分词，在重复或低熵区域使用粗粒度压缩

Result: 在WikiText-103和LongBench等长文本建模基准测试中，SemToken实现了最多2.4倍的token数量减少和1.9倍的加速，困惑度和下游任务准确率几乎没有下降
...
<details>
  <summary>Details</summary>
Motivation: 现有的PRMs主要针对通用或STEM领域训练，在金融等专业领域中表现不足，因为金融推理更加结构化、符号化且对事实和监管正确性敏感。

Method: 开发了Fin-PRM，一个领域专业化的轨迹感知PRM，整合步骤级和轨迹级奖励监督，支持离线/在线奖励学习，应用于监督微调、强化学习和测试时推理。

Result: 在CFLUE和FinQA基准测试中，Fin-PRM在轨迹选择质量上持续优于通用PRMs和强领域基线，下游模型性能提升显著：监督学习12.9%、强化学习5.2%、测试时性能5.1%。

Conclusion: 领域专业化的奖励建模对于使LLMs与专家级金融推理对齐具有重要价值，Fin-PRM为金融领域的中间推理监督提供了有效解决方案。
...
TL;DR: SPARK是一种无需训练的即插即用方法，通过在通道级别对KV缓存进行剪枝来减少内存使用，同时动态恢复被剪枝的条目，有效解决了长上下文推理中的KV缓存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要沿时间轴压缩KV缓存，但忽略了特征维度（通道轴）上的细粒度重要性变化，限制了效率和模型精度的平衡能力。

Method: 提出SPARK方法，在通道级别应用非结构化稀疏性对KV进行剪枝，在注意力分数计算期间动态恢复被剪枝的条目，该方法与现有KV压缩和量化技术正交兼容。

Result: 在相同内存预算下可处理更长序列；相同序列长度下KV缓存存储减少30%以上；即使80%的激进剪枝比例，性能下降也小于5%。

Conclusion: SPARK通过减少通道级冗余，有效平衡了效率和准确性，为长上下文推理提供了实用的解决方案。
...
Result: 在医学、法律和金融QA基准测试中，S2K始终优于现有方法，并以显著更低的成本达到领域预训练LLMs的水平。

Conclusion: S2K通过渐进式知识获取策略有效解决了领域特定知识的长尾分布问题，实现了成本效益高的领域知识内化。
...
TL;DR: SEER方法通过逐步检索经验池来解决LLM在多步骤工具调用中的困难，无需人工设计演示，在多个基准测试中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工设计任务特定演示或从精选库中检索，需要大量专家工作，随着工具多样性和任务难度增加，提示工程变得复杂低效

Method: 提出自引导方法Stepwise Experience Recall (SEER)，从持续更新的经验池中进行细粒度逐步检索，增量式扩充经验池

Result: 在ToolQA基准上平均提升6.1%(简单问题)和4.7%(困难问题)，在τ-bench上使用Qwen2.5-7B和72B模型分别获得7.44%和23.38%的准确率提升

Conclusion: SEER方法能够有效解决LLM在多步骤工具调用中的挑战，通过经验池的持续学习和检索实现性能的持续改进
...
TL;DR: 本研究探讨了在大语言模型自动评估生成任务时选择性使用检查表的有效性，发现选择性使用在成对比较任务中表现更好，但在直接评分中效果不一致。


<details>
  <summary>Details</summary>
Motivation: 大语言模型自动评估生成任务时面临标准模糊的挑战，自动生成检查表是一种有前景的方法，但其有效性尚未充分探索。

Method: 使用六种方法生成检查表，在八种不同规模的模型上进行实验，通过成对比较和直接评分任务评估检查表效果，并分析检查表项与人工评估的相关性。

Result: 选择性使用检查表在成对比较设置中能提高评估性能，但在直接评分中效果不一致；即使与人工评分相关性低的检查表项也反映了人工编写的标准，表明人工评估存在潜在不一致性。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在边缘设备部署时面临内存瓶颈，主要来自词汇相关组件（嵌入和语言建模头）。现有静态词汇剪枝方法存在信息损失和灵活性不足的问题。

Method: 基于词汇局部性原理和计算特性不对称性，提出VocabTailor框架：1）通过卸载嵌入减少内存占用；2）为LM头设计混合静态-动态词汇选择策略，实现按需加载。

Result: 在多样化下游任务中，VocabTailor将词汇相关组件内存使用减少高达99%，任务性能几乎无退化，显著优于现有静态词汇剪枝方法。

Conclusion: VocabTailor通过动态词汇选择有效解决了SLM内存瓶颈问题，为边缘设备部署提供了高效的内存优化方案。
...
TL;DR: 提出了WangchanThaiInstruct数据集，这是一个人工编写的泰语指令数据集，用于评估和指令调优，涵盖4个专业领域和7种任务类型。研究表明，使用本地监督数据微调的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语指令跟随方面表现出色，但在泰语等低资源语言中的性能尚未充分探索。现有基准测试通常依赖翻译，缺乏实际应用所需的文化和领域特定细微差别。

Method: 通过多阶段质量控制流程创建人工编写的泰语数据集，涵盖专业领域和任务类型。进行两项研究：零样本评估显示在文化和专业特定任务上的性能差距，以及指令调优研究，通过消融实验分离本地监督的效果。

Result: 在WangchanThaiInstruct上微调的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。零样本评估揭示了在文化和专业特定任务上的性能差距。

Conclusion: 研究结果强调了需要基于文化和专业背景的指令数据，以改善低资源、语言多样化环境中LLM的对齐性能。本地监督对于提升模型在特定语言和文化环境中的表现至关重要。
...
Method: 使用SWORDS算法（同义词替换单词），在保持句子语义的前提下替换选定单词为翻译版本，考虑词性因素

Result: 构建的CS-FLEURS语料库在可懂度和自然度方面表现优异，与现有数据集相当

Conclusion: 该方法能推动代码切换语音技术发展，实现更具包容性的多语言系统
...
Result: 教师角色LLMs表现出比人类教师更理想化和极化的人格特征，在抽象道德推理方面优秀但在情感复杂情境中表现不佳。推理能力更强的模型更容易受到有害提示注入的影响。

Conclusion: 本文提出了首个评估教师角色LLMs伦理和心理对齐的基准，揭示了能力与安全性之间的悖论，为教育AI的发展提供了重要参考。
...
TL;DR: CARE是一个解决RAG系统中上下文-记忆冲突的新方法，通过上下文评估器和软提示技术来识别不可靠的外部上下文，提高模型在冲突情况下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成(RAG)系统在处理外部检索上下文与LLM内部参数知识冲突时表现不佳，无法有效解决上下文-记忆冲突问题。

Method: 提出CARE框架，包含上下文评估器和基础LLM。上下文评估器通过压缩记忆token嵌入和基于对抗软提示的训练，学习识别不可靠上下文并引导推理朝向更可靠的知识源。
...
<details>
  <summary>Details</summary>
Motivation: 自然语言中时间上下文往往不明确，现有大语言模型在处理需要时间常识推理的文本生成任务时表现不佳，因此需要自动挖掘时间常识来增强语言模型的鲁棒性。

Method: 提出了一个时间常识提取流程，利用大语言模型从SAMSum和RealNews语料库中自动挖掘时间常识，构建TComQA数据集，并通过众包验证数据质量。

Result: TComQA数据集提取时间常识的精确度超过80%，基于该数据集训练的模型在时间问答任务上表现优于在现有数据集上微调的大语言模型。

Conclusion: 该方法能够有效自动挖掘时间常识，构建高质量数据集，提升语言模型在时间推理任务上的性能。
...
Method: 处理短固定宽度窗口（约一个音素长度），独立分析，通过监督和自监督训练学习所有语言共有的基础声学模式
...
TL;DR: 提出了EDAS元度量方法，将知识图谱补全模型在多个数据集和评估指标上的性能整合为单一标准化分数，解决传统评估中指标冲突和跨数据集比较困难的问题。
...
Method: 提出基于平均解距离的评估方法（EDAS），通过计算模型性能与平均解的相对距离，将多数据集、多指标的性能综合为0-1范围内的标准化分数。

Result: 在FB15k-237和WN18RR等基准数据集上的实验表明，EDAS能够有效整合多指标、多数据集性能，提供一致、稳健且可推广的评估框架。

Conclusion: EDAS提供了一个统一、可解释的元度量，支持更明智的模型选择，促进了跨数据集评估的公平性，解决了知识图谱补全模型评估中的不一致性问题。
...
TL;DR: 本文首次系统综述了283个大语言模型评测基准，将其分为通用能力、领域特定和目标特定三类，指出了当前基准存在的数据污染、文化偏见等问题，并提供了未来基准创新的设计范式。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的快速发展，各种评测基准大量涌现。基准作为模型性能的量化评估工具，不仅是衡量模型能力的核心手段，也是指导模型发展方向和推动技术创新的关键要素。

Method: 系统性地回顾了大语言模型评测基准的现状和发展，将283个代表性基准分为三类：通用能力基准（涵盖核心语言学、知识、推理等）、领域特定基准（自然科学、人文社科、工程技术等）和目标特定基准（风险、可靠性、智能体等）。

Result: 发现当前基准存在数据污染导致的分数膨胀、文化和语言偏见造成的不公平评估、缺乏过程可信度和动态环境评估等问题。

Conclusion: 为未来基准创新提供了可参考的设计范式，旨在解决现有基准的局限性，推动更全面、公平和有效的大语言模型评估体系发展。
...
Method: 提出了MultiTrust-X基准，定义了一个三维框架：五个可信度维度（真实性、鲁棒性、安全性、公平性、隐私）、两种新型风险类型（多模态风险和跨模态影响）、多种缓解策略（数据、模型架构、训练和推理算法）。基于该分类法，包含32个任务和28个精选数据集。

Result: 实验发现当前模型存在显著漏洞，包括可信度与通用能力之间的差距，以及多模态训练和推理对基础LLM潜在风险的放大。现有缓解策略存在局限性，虽然某些方法在特定方面有改进，但很少有方法能有效解决整体可信度问题，且许多方法会引入意外的权衡而损害模型效用。
...
TL;DR: 提出基于置信度调制的自适应推测解码框架，通过熵和边界不确定性动态调整推测生成token数量，提高解码效率同时保持输出质量
...
Method: 基于信息论的置信度调制框架，利用drafter输出分布的熵和边界不确定性度量，动态调整每轮推测生成的token数量，并使用相同置信度信号调制验证过程
...
TL;DR: 研究表明，更大的词汇表通过降低token化文本的复杂度来提升语言模型性能，主要是通过减少高频词的不确定性来实现，而不是缓解词汇不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 探究大型词汇表对语言模型性能提升的真正原因，现有实践倾向于使用更大的词汇表但具体机制不明确。

Method: 在固定数据、计算和优化条件下，将词汇表从24K扩展到196K，量化token化文本复杂度，分析词汇频率分布和损失分解。

Result: 大于24K的词汇表主要通过降低前2500个高频词的不确定性来减少交叉熵，尽管低频词损失上升。词汇不平衡被模型利用而非损害性能。

Conclusion: 词汇表大小的好处源于降低token化文本复杂度，这为tokenizer和模型的协同设计提供了原则性指导，并阐明了预训练中的损失动态。
...
Result: 建立了基于证据的LLM文本生成的统一分类体系，识别了该领域的独特特征和代表性方法，为后续研究提供了系统性的分析框架。

Conclusion: 论文指出了该领域的开放挑战，并为未来工作规划了有前景的研究方向，为基于证据的LLM文本生成研究提供了重要的理论基础和实践指导。
...
TL;DR: 该论文提出了首个专门评估大型音频语言模型在处理音频-文本冲突信息时偏好的基准测试MCR-BENCH，发现模型存在显著的文本偏向性，经常忽视音频证据，导致音频中心任务性能下降。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型在音频感知方面有所增强，但它们在处理音频和文本模态间冲突信息时的表现尚未得到充分研究，需要系统评估其信息优先级选择机制。

Method: 构建MCR-BENCH基准测试，通过不一致的音频-文本对评估模型表现，分析影响因素，探索监督微调缓解策略，并研究模型置信度模式。

Result: 研究发现LALMs存在明显的文本偏向性，在模态冲突时经常忽略音频证据，导致音频任务性能显著下降，且模型即使在矛盾输入下也表现出持续过度自信。

Conclusion: 需要在训练过程中改进模态平衡，开发更复杂的融合机制，以增强模型在处理冲突多模态输入时的鲁棒性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语音语言模型（LSLM）发展受限于碎片化的架构和缺乏透明度，模型权重发布时往往缺少对应的训练数据和配置，阻碍了系统比较和研究的可复现性。
...
Conclusion: 通过发布完整的数据、基准和模型堆栈，LLaSO建立了一个基础性的开放标准，旨在统一研究努力并加速LSLM领域的社区驱动进展。
...
Method: 提供隐私保护语言建模方法的深度综述研究，系统分析不同方法的优缺点

Result: 该研究为隐私保护语言建模的持续研究做出贡献，提供了有价值的见解

Conclusion: 研究概述了未来研究方向，有助于推动隐私保护语言建模领域的发展
...
<details>
  <summary>Details</summary>
Motivation: 心理健康障碍是全球危机，现有数据集在识别主动寻求帮助的个体方面存在关键空白，需要专门针对求助行为的数据集。
...
TL;DR: 这篇论文研究了源语言中无对应词汇的翻译方法，通过图书调研方式提出了专业的翻译规则，并将达尔赫语和乌兹别克语中的25个无对应词汇翻译成英语和俄语。


<details>
  <summary>Details</summary>
Motivation: 因为不同语言在食物、服装、文化和传统词汇等方面存在语言缺位现象，这些无对应词汇宽成理解障碍，需要专业的翻译方法来确保语言理解的准确性。

Method: 采用图书调研方法，研究了从源语言到目标语言的无对应词汇翻译的各种方法和规则。

Result: 研究提出了专业的无对应词汇翻译方法，并成功将达尔赫语和乌兹别克语中的25个无对应词汇翻译成英语和俄语。

Conclusion: 该研究为解决语言间无对应词汇的翻译问题提供了有效的方法论和实践案例，对于提高翻译质量和消除语言障碍具有重要意义。
...
TL;DR: RadReason是一个新颖的放射学报告评估框架，通过细粒度错误类型分析和可解释的评分机制，解决了现有评估方法在临床实用性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告自动评估存在缺乏临床基础、不可解释和粒度粗糙的问题，限制了在真实临床工作流程中的应用价值。

Method: 基于Group Relative Policy Optimization，引入两个关键创新：子分数动态加权（根据实时F1统计自适应优先处理临床挑战性错误类型）和多数引导优势缩放（基于子分数一致性调整策略梯度更新）。

Result: 在ReXVal基准测试中，RadReason超越了所有现有离线指标，达到与GPT-4评估相当的性能，同时保持可解释性、成本效益和临床部署适用性。

Conclusion: RadReason提供了一个临床基础扎实、可解释且细粒度的放射学报告评估解决方案，有望在真实临床环境中发挥重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 个性化营销在提升客户参与度和业务增长方面具有重要作用，现有研究主要集中在推荐系统和个性化广告，而个性化报价生成具有显著提升转化率和客户满意度的潜力
...
Conclusion: 对比学习目标在推进个性化营销方面具有有效性，SLM4Offer模型通过自适应学习行为增强了模型的泛化能力
...
TL;DR: 本文质疑大型语言模型在处理用户主观浏览行为时的有效性，提出了一种针对异构用户群体的聚类语言模型训练方法HeTLM，发现小型模型在特定tokenizer下优于大型模型，且异构参数集能提高平均性能并降低方差。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理用户主观行为和偏好时可能存在局限性，特别是在用户浏览行为具有高度异质性和主观性的场景下，需要探索更有效的建模方法。

Method: 提出HeTLM（Heterogeneity aware Training of Language Model）方法，使用页面级tokenizer训练小型语言模型，并为不同用户聚类分配异构参数集。

Result: 实验表明：1）使用页面级tokenizer的小型模型优于大型预训练或微调模型；2）异构聚类特定参数集的HeTLM优于同家族单一模型；3）实现了更高的平均性能和更低的方差，提高了对齐效果。

Conclusion: 针对用户主观浏览行为，小型专业化模型和异构参数策略比通用大型语言模型更有效，能够更好地捕捉用户异质性并提高个性化对齐性能。
...
TL;DR: 本研究提出使用训练数据影响力作为难度指标来构建课程学习，相比传统人工定义的难度指标，在语言模型预训练中能带来超过10个百分点的性能提升
...
Method: 使用训练数据影响力作为新的难度指标来对训练示例进行排序，该指标能更准确地估计单个训练示例对模型输出的影响

Result: 采用模型中心化难度指标的课程学习方法在基准测试中比随机顺序训练的性能高出超过10个百分点

Conclusion: 课程学习对语言模型预训练确实有益，但需要采用更符合模型训练特性的难度定义方式，而不是传统的人类中心化难度指标
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏对小型语言模型性能和环境影响系统性评估的基准测试，需要建立全面评估框架来填补这一空白。

Method: 开发了SLM-Bench基准测试，在4种硬件配置下评估15个SLM模型，使用23个数据集覆盖14个领域，量化11个正确性、计算和能耗指标。

Result: 评估结果显示不同SLM模型在准确性和能效方面存在显著权衡，部分模型在准确性上表现优异，而其他模型在能效方面更优。
...
<details>
  <summary>Details</summary>
Motivation: 现有社会身份检测数据集主要是英语中心、单标签且关注粗粒度身份类别，而社会身份通常由特定文化背景塑造并通过特定语言使用表达，需要针对希伯来语等非英语政治语境的多标签细粒度身份检测数据集。

Method: 构建包含5536个以色列政客Facebook帖子句子的多标签希伯来语语料库，手动标注12种基于调查数据的细粒度社会身份，对比多标签和单标签编码器以及20-90亿参数生成式大语言模型。

Result: 希伯来语调优的大语言模型表现最佳（宏观F1=0.74），应用于政客Facebook帖子和议会演讲分析，发现了受欢迎程度、时间趋势、聚类模式和性别相关身份表达的差异。

Conclusion: HebID为研究希伯来语社会身份提供了全面基础，可作为其他非英语政治语境类似研究的模型，并能够比较精英话语中呈现的身份与公众身份优先事项之间的差异。
...
Result: 在通用、数学和编程任务上一致优于现有扩散语言模型，展现出优越的规划能力、推理灵活性，支持任意顺序生成、填充能力和可调节的质量-速度权衡。

Conclusion: Dream 7B证明了扩散建模在语言生成任务中的有效性，为扩散式语言建模的进一步研究提供了基础模型和指令微调版本。
...
TL;DR: 首个大规模政治去合法化话语(PDD)计算研究，构建希伯来语语料库并开发两阶段分类模型，发现PDD在过去三十年显著增加，社交媒体比议会辩论更普遍，右翼政治人物使用更多。


<details>
  <summary>Details</summary>
Motivation: 研究政治去合法化话语(PDD)的计算分析方法，以理解民主话语中象征性攻击政治实体规范有效性的现象。

Method: 构建包含10,410个句子的希伯来语语料库，手动标注1,812个PDD实例；开发两阶段分类管道，结合微调编码器模型和解码器LLM。

Result: 最佳模型(DictaLM 2.0)在二元PDD检测中F1得分0.74，在去合法化特征分类中宏观F1得分0.67；发现PDD在过去三十年显著增加，社交媒体比议会辩论更普遍，男性比女性政治人物使用更多，右翼行为者倾向更强。

Conclusion: 自动化PDD分析对于理解民主话语具有可行性和价值，特别是在选举活动和重大政治事件期间表现出明显的高峰。
...
TL;DR: SafetyFlow是一个首个完全自动化的LLM安全评估基准构建系统，通过7个专业代理在4天内自动构建包含23,446个查询的低冗余安全基准数据集
...
Method: 设计基于代理流的自动化系统SafetyFlow，配备7个专业代理和多功能工具，实现无需人工干预的基准构建流程

Result: 构建了SafetyFlowBench数据集，包含23,446个低冗余查询，对49个先进LLM进行安全评估，验证了系统的有效性和效率

Conclusion: SafetyFlow首次实现了LLM安全基准的完全自动化构建，显著降低了时间和资源成本，为LLM安全评估提供了高效可靠的解决方案
...
Result: 能够以较低成本生成类似的领域特定响应

Conclusion: 小型专门化模型是一种成本效益更高的解决方案，适用于具体的高价值应用场景
...
Method: 提出SDGO强化学习框架，使用模型自身的判别能力作为奖励信号，通过迭代自我改进来增强生成安全性。训练阶段不需要额外标注数据或外部模型。

Result: SDGO显著提高了模型安全性，在保持通用基准测试有用性的同时，对分布外越狱攻击表现出更强的鲁棒性。通过少量判别样本就能进一步增强生成能力。

Conclusion: 通过对齐LLM的判别和生成能力，SDGO实现了更紧密的能力耦合，为LLM安全提供了一种有效且实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于学术文献快速增长，手动撰写科学综述变得不可行，而大语言模型在自动化这一过程中缺乏标准化基准和评估协议，阻碍了该领域的发展。

Method: 构建SurGE基准测试，包括主题描述、专家撰写的综述及其引用参考文献的测试实例集合，以及超过100万篇论文的大规模学术语料库作为检索池，并提出自动化评估框架从四个维度评估生成综述。

Result: 对多种基于LLM的方法评估表明，即使使用先进的自反思框架，综述生成仍然极具挑战性，凸显了该任务的复杂性。

Conclusion: 科学综述生成是一个复杂的任务，需要持续的研究努力，作者开源了所有代码、数据和模型以促进该领域发展。
...
Method: 提出Pos2Distill框架，利用位置诱导的差异来抵消位置偏差本身。针对检索和推理范式设计了两个专门实例：Pos2Distill-R¹和Pos2Distill-R²，通过知识蒸馏将优势位置的能力转移到劣势位置。

Result: 使用Pos2Distill方法在长上下文检索和推理任务中实现了增强的均匀性和显著的性能提升。两个专门系统都表现出强大的跨任务泛化能力，同时在各自任务上达到优越性能。
...
TL;DR: 本文对孟加拉语词干提取方法进行了全面调查，指出了现有研究的空白、评估方法的不足，并提出了未来发展方向建议。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第七大语言，由于资源有限和标注数据集缺乏而面临数字代表性不足的问题。词干提取作为语言分析的关键预处理步骤，对于像孟加拉语这样的低资源、高屈折性语言至关重要。

Method: 采用文献综述和调查方法，全面分析现有的孟加拉语词干提取方法，识别研究空白和评估方法的问题。

Result: 发现现有文献存在显著空白，先前研究存在不连续性，可复现的实现稀缺，评估方法需要更相关的指标。
...
Conclusion: 需要更智能的方法来策略性利用多模态信息，SUMEI提供了一种数据驱动的方法来优化多图像在电商任务中的使用效果。
...
TL;DR: Deep-DxSearch是一个基于强化学习的端到端代理RAG系统，通过构建大规模医学检索语料库和定制化奖励机制，显著提升了医学LLM的诊断准确性和推理可追溯性。


<details>
  <summary>Details</summary>
Motivation: 当前医学大语言模型在诊断中存在知识鸿沟和幻觉问题，传统检索增强方法对外部知识利用不足且反馈推理可追溯性差，需要更有效的解决方案。
...
Conclusion: Deep-DxSearch通过强化学习驱动的代理RAG框架有效解决了医学诊断中的知识鸿沟问题，提供了更可靠和精确的初步诊断支持，奖励设计和检索语料库组件对性能提升至关重要。
...
TL;DR: ReasonZoo基准测试显示，工具集成推理(TIR)能显著提升大语言模型在数学和非数学推理任务中的表现和效率，减少过度思考。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在推理任务上取得进展，但在精确计算方面仍有不足。工具集成推理(TIR)被提出作为解决方案，但其泛化效果和对模型推理行为的改善尚不明确。

Method: 提出了ReasonZoo基准测试，涵盖9个不同推理类别，并设计了两个新指标：性能感知成本(PAC)和性能-成本曲线下面积(AUC-PCC)来评估推理效率。

Result: TIR模型在数学和非数学任务中均优于非TIR模型，推理效率提升，表现为PAC和AUC-PCC指标的改善，表明减少了过度思考，推理更加精简。
...
Result: 实验表明即使最先进的LLM成功率也低于60%，详细消融和错误分析揭示了工具协调中的主要挑战、不同的失败模式以及令牌使用效率低下。
...
<details>
  <summary>Details</summary>
Motivation: 热图回归方法在面部关键点检测中广泛使用，但argmax操作不可微分，传统使用Soft-argmax作为近似。作者质疑这一长期选择，认为可能存在更好的替代方案。

Method: 提出基于经典结构化预测框架的替代训练目标，不使用Soft-argmax，而是采用不同的可微分训练方法来实现端到端训练。

Result: 在WFLW、COFW和300W三个面部关键点基准测试中达到最先进性能，训练收敛速度提升2.2倍，同时保持更好或相当的准确率。

Conclusion: Soft-argmax并非实现高性能的唯一途径，结构化预测框架提供了有效的替代方案，在收敛速度和准确率方面都有显著优势。
...
TL;DR: 这篇论文提出了一种新的图像分类方法，通过将图卷积神经网络（GCNs）与Voronoi图结合，以图形结构表示图像，显著提升了分类准确性和预处理效率。


<details>
  <summary>Details</summary>
Motivation: 传统的卷积神经网络（CNNs）在处理复杂数据结构时有限，需要更有效地模型图像中的关系性数据。

Method: 将图像表示为图形（像素或区域作为顶点），使用Delaunay三角剖分优化图形表示，然后通过GCNs与Voronoi图结合进行分析和分类。

Result: 在多个标准数据集上实现了显著的分类准确性提升，特别是在复杂场景和细粒度分类任务中超越了最先进方法。

Conclusion: 这项研究不仅提供了图像分类的新视角，还扩展了图形基学习在计算机视觉和非结构化数据分析中的应用潜力。
...
TL;DR: YOPO是一个单阶段、基于查询的RGB图像9自由度姿态估计框架，将物体检测和姿态估计统一起来，无需额外数据，在三个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖伪深度、CAD模型或多阶段级联，需要一个更简单的仅RGB方法，直接在类别级别学习，统一物体检测和9自由度姿态估计。

Method: 基于transformer检测器，增加轻量级姿态头、边界框条件平移模块和6D感知匈牙利匹配成本，仅使用RGB图像和类别级姿态标签进行端到端训练。

Result: 在REAL275数据集上达到79.6% IoU50和54.1% 10°10cm指标，超越先前RGB方法，缩小与RGB-D系统的差距。

Conclusion: YOPO证明了单阶段RGB方法在类别级9自由度姿态估计中的有效性，实现了检测和姿态估计的高性能统一。
...
TL;DR: 统一的人脸活体检测框架，通过对比学习自动匹配的真实-攻击图片对，同时处理物理和数字攻击，达到2.10%的平均分类错误率。


<details>
  <summary>Details</summary>
Motivation: 现代人脸识别系统仍容易受到物理和数字攻击的伪造，传统方法需要使用不同模型分别处理这两类攻击，导致系统复杂度增加、推理延迟和存在结合攻击的风险。

Method: 提出Paired-Sampling Contrastive Framework，一种统一训练方法，利用自动匹配的真实人脸和攻击图片对，学习不依赖于特定模态的活体线索。

Result: 在第6届人脸防伪造挑战赛统一物理-数字攻击检测基准上，方法达到2.10%的平均分类错误率(ACER)，超过之前的解决方案。框架轻量(4.46 GFLOPs)，训练时间少于1小时。

Conclusion: 该统一框架能够高效地同时处理物理和数字伪造攻击，具有较低的计算复杂度和短短的训练时间，适合实际部署应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型的对抗攻击通常产生低质量图像且计算资源需求大，扩散模型虽然能生成高质量图像但需要数百个采样步骤
...
Conclusion: TAIGen实现了高效高质量的对抗图像生成，在保持视觉质量的同时最大化目标模型的误分类，是最具影响力的攻击方法
...
<details>
  <summary>Details</summary>
Motivation: 现有CVP方法主要局限于掩码域的可逆策略，未能充分利用RGB域的潜力，需要一种跨域的可逆建模方法来减少不确定性
...
Conclusion: RUN++通过可逆展开网络和生成式细化的协同作用，为CVP系统提供了一个新的强大框架，能够跨域处理不确定性并保持计算效率
...
Result: 在11,694标注帧的数据集上，达到74.47% mIoU和83.63% mF1分割精度，仅3.348M参数和3.428G FLOPs，推理速度114.9 FPS，饮食分类准确率100%。
...
TL;DR: CurveFlow通过引入曲率正则化技术，学习非线性轨迹来提高文本到图像生成的语义一致性，在MS COCO数据集上实现了最先进的性能。
...
Result: 在MS COCO 2014和2017上的实验表明，CurveFlow在文本到图像生成方面达到了最先进的性能，显著优于标准整流流变体和其他非线性基线，在BLEU、METEOR、ROUGE和CLAIR等语义一致性指标上表现尤为突出。
...
TL;DR: HiRQA是一个自监督的无参考图像质量评估框架，通过分层排序和质量对齐实现质量感知嵌入，无需参考图像或主观标签，在合成和真实失真上都表现出优异的泛化性能。
...
Result: 仅在合成失真上训练的HiRQA能有效泛化到真实退化场景（如镜头光晕、雾霾、运动模糊、低光照），在合成和真实基准测试中都达到SOTA性能，轻量版HiRQA-S推理时间仅3.5ms。

Conclusion: HiRQA通过自监督的层次化排序和质量对齐方法，解决了NR-IQA的泛化问题，无需参考图像或主观标签，在多种失真类型上表现出强大的泛化能力和实时部署潜力。
...
<details>
  <summary>Details</summary>
Motivation: 多视角3D重建在应急响应等关键应用中需要近实时性能，但边缘环境的动态性和操作逆境会导致时空相关中断，影响重建质量

Method: 采用投资组合理论启发的方法，通过遗传算法解决优化问题，选择相机组合以应对可能的系统中断

Result: 在公开和自定义3D数据集上的实验表明，该方法相比传统基线策略能更好地保证在时空中断下的可靠3D重建

Conclusion: 所提出的相机选择策略能够有效应对边缘环境中的时空相关中断，确保多视角3D重建的质量可靠性
...
TL;DR: 提出XDR-LVLM框架，利用视觉语言大模型实现高精度糖尿病视网膜病变诊断，并生成自然语言解释，在准确性和可解释性方面均达到先进水平。
...
Method: 整合专用医学视觉编码器和LVLM核心，采用多任务提示工程和多阶段微调，深入理解眼底图像病理特征并生成包含DR严重程度分级、关键病理概念识别和详细解释的综合诊断报告。

Result: 在DDR数据集上达到84.55%的平衡准确率和79.92%的F1分数，概念检测达到77.95% BACC和66.88% F1，人类评估确认生成解释具有高流畅性、准确性和临床实用性。

Conclusion: XDR-LVLM通过提供稳健且可解释的见解，成功弥合了自动化诊断与临床需求之间的差距，展示了在医疗AI中实现透明度和可信度的重要性。
...
TL;DR: MeSS方法通过改进图像扩散模型，使用级联外绘ControlNet、AGInpaint和GCAlign模块，从城市网格模型生成高质量、风格一致的三维室外场景纹理


<details>
  <summary>Details</summary>
Motivation: 城市网格模型缺乏逼真纹理，限制了在虚拟城市导航和自动驾驶中的应用，需要生成高质量且视觉一致的三维场景

Method: 三阶段流程：1)使用级联外绘ControlNet生成几何一致的稀疏视图；2)通过AGInpaint传播更密集的中间视图；3)使用GCAlign模块全局消除视觉不一致性。同时基于网格表面初始化3D高斯溅射场景重建

Result: 在几何对齐和生成质量方面优于现有方法，生成场景可通过重光照和风格迁移技术以不同风格渲染

Conclusion: MeSS方法成功解决了城市网格模型纹理生成问题，为虚拟城市导航和自动驾驶提供了高质量的视觉一致三维场景
...
TL;DR: 提出一种基于学习的方法来推导规则智能体的对抗行为，以在自动驾驶等安全关键应用中创建故障场景


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等安全关键应用中，准确建模周围规则智能体行为至关重要，现有方法使用多种行为建模策略和IDM模型，但需要测试这些模型在对抗情况下的鲁棒性

Method: 使用基于学习的方法来推导规则智能体的对抗行为，通过对抗智能体来引发故障场景

Result: 评估了对抗智能体对抗所有规则智能体的效果，显示累积奖励显著下降，证明了该方法能有效创建故障情况

Conclusion: 该方法能够有效识别规则智能体模型的脆弱性，为安全关键系统的鲁棒性测试提供了有效工具
...
TL;DR: DyMorph-B2I是一个针对肾脏病理学的动态形态学引导的二元到实例分割管道，通过整合分水岭、骨架化和形态学操作，实现了比传统方法更优的实例分离效果。


<details>
  <summary>Details</summary>
Motivation: 现有的肾脏病理学数据集和自动化方法主要提供二元（语义）分割掩码，限制了后续形态学定量分析的精确性。传统后处理技术在分离复杂连接的肾脏组织结构时效果有限。

Method: 提出DyMorph-B2I管道，在统一框架中整合分水岭、骨架化和形态学操作，并采用自适应几何细化和可定制的超参数调优，针对不同类型的肾脏功能单元进行优化。

Result: 实验结果表明，该方法优于单个传统方法和简单组合方法，能够稳健地分离二元掩码中的粘连和异质结构，实现更优越的实例分离效果。

Conclusion: DyMorph-B2I管道能够促进更准确的肾脏病理学形态计量分析，该工具已公开发布在GitHub上供研究使用。
...
TL;DR: 提出了STAGNet模型，通过改进时空特征提取和循环网络聚合，在仅使用行车记录仪视频的情况下提升事故预测性能，在多个数据集上超越了现有方法的精度和预测时间。
...
Method: 采用改进的时空特征提取方法，通过循环网络聚合特征，基于图神经网络架构构建STAGNet模型。

Result: 在三个公开数据集上测试，STAGNet在平均精度和平均碰撞时间两个指标上都优于现有方法，在跨数据集测试中也表现良好。

Conclusion: STAGNet证明了仅使用行车记录仪视频就能实现有效的事故预测，为低成本、易部署的ADAS系统提供了可行方案。
...
TL;DR: TriMM是首个前馈式3D原生生成模型，通过整合RGB、RGBD和点云等多模态数据，利用协作多模态编码和三平面潜在扩散模型，显著提升3D资产的纹理和几何细节质量。
...
Conclusion: TriMM通过有效利用多模态数据，成功解决了现有3D生成模型的局限性，为3D内容生成提供了更全面和高质量的解决方案。
...
Conclusion: 该框架能够有效避免类间冲突、减少原型漂移，并确保特征具有变换不变的语义信息和紧凑的簇内分布
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机视觉语言导航存在轨迹长、机动性复杂的问题，需要人工干预或过于详细的指令。利用无人机高机动性提供多粒度视角的同时保持可学习运动空间。

Method: 提出双高度无人机协作框架：高空无人机负责环境推理，低空无人机负责精确导航。使用多模态大语言模型(Pilot-LLM)进行目标推理，轻量级多阶段策略进行导航和目标定位，仅交换坐标信息确保效率。

Result: 构建了包含13,838条协作轨迹的HaL-13k数据集，包含未见地图和未见对象验证集，系统评估模型在新环境和陌生目标上的泛化能力。

Conclusion: 双无人机协作方法能够有效结合高低空优势，通过最小信息交换实现高效协作导航，为复杂户外环境下的无人机自主导航提供了新思路。
...
TL;DR: 这篇论文提出了跳跃步采样机制，通过跳过多个中间去噪步骤来加速传播模型的生成过程，而无需采用非马尔可夫过程。


<details>
  <summary>Details</summary>
Motivation: 虽然传播模型在各种生成任务中取得了最先进的结果，但其序列生成过程需要长序列步骤逐步生成，效率较低。现有方法如DDIM尝试减少采样步骤，但仍需构造非马尔可夫过程，但对于原始传播过程是否能以马尔可夫方式实现同样效率的问题仍有待解决。

Method: 提出跳跃步采样机制，跳过多个中间去噪步骤，与标准传播推理的逐步精炼形成对比。证明这种机制来自于与标准传播模型相同的训练目标，表明通过跳跃步采样进行加速是预训练传播模型的内在特性。还提出了一种增强生成方法，将加速采样技术与DDIM相结合。

Result: 在多个流行的预训练传播模型上进行了大量实验，包括OpenAI ADM、Stable Diffusion和Open Sora模型。实验结果显示，该方法能够在显著减少采样步骤的情况下实现高质量的生成。

Conclusion: 这篇论文确认了原始传播过程可以通过马尔可夫方式实现高效采样，无需借助非马尔可夫过程。提出的跳跃步采样机制是预训练传播模型的内在特性，能够在保持高生成质量的同时显著提高生成效率。
...
Method: 提出三方面创新：(1)多功能编码框架统一不同目标需求的编码模式；(2)基于增强上下文学习的交互编码代理；(3)首个专用基准测试IIC-bench。
...
TL;DR: Ano-NAViLa是一个基于预训练视觉-语言模型的病理异常检测方法，通过整合正常和异常病理知识，在计算病理学中实现了最先进的异常检测和定位性能。
...
Method: 基于预训练视觉-语言模型构建，加入轻量级可训练MLP，整合正常和异常病理知识，通过图像-文本关联提供可解释性。

Result: 在两个不同器官的淋巴结数据集上评估，Ano-NAViLa在异常检测和定位方面达到了最先进性能，超越了竞争模型。

Conclusion: Ano-NAViLa通过知识增强的视觉-语言模型方法，有效解决了病理异常检测的挑战，提供了准确、鲁棒且可解释的解决方案。
...
Conclusion: 冗余分配策略通过一对多监督实现了数量丰富和几何多样化的拓扑监督，有效解决了现有方法监督范围有限的问题。
...
TL;DR: DesignCLIP是一个基于CLIP视觉语言模型的设计专利分析框架，通过类别感知分类和对比学习，结合生成详细图像描述和多视图学习，在专利分类和检索任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统设计专利分析依赖图像数据，但专利草图往往缺乏完整的视觉上下文和语义信息，导致在先技术搜索时产生歧义。需要利用先进的视觉语言模型来提高专利分析的可靠性和准确性。

Method: 利用CLIP模型开发统一框架DesignCLIP，采用类别感知分类和对比学习，使用生成的详细图像描述和多视图图像学习来处理专利数据的独特特征。

Result: DesignCLIP在所有专利相关任务（包括专利分类、检索和多模态检索）上都一致优于基线和最先进模型，证明了多模态方法在专利分析中的潜力。
...
TL;DR: TPA是一种基于基础图像-文本模型和提示感知对比学习的胎儿先天性心脏病超声视频分类方法，通过时间特征提取和文本提示对齐，结合条件变分自编码器进行不确定性量化，在CHD检测和心脏功能评估任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前先天性心脏病超声检测方法存在图像噪声、探头位置变异性问题，现有机器学习方法往往忽略时间信息、仅限于二元分类且缺乏预测校准，需要开发能够处理时间动态、多分类且具有临床可靠性的自动化方法。

Method: 提出时间提示对齐(TPA)框架：1)使用图像编码器提取视频片段每帧特征；2)通过可训练时间提取器聚合特征捕捉心脏运动；3)通过边界铰链对比损失将视频表示与类别特定文本提示对齐；4)引入条件变分自编码器风格调制模块学习潜在风格向量来调制嵌入并量化分类不确定性。

Result: 在CHD检测任务上达到85.40%的宏观F1分数，预期校准误差降低5.38%，自适应ECE降低6.8%；在EchoNet-Dynamic三分类任务上宏观F1提升4.73%(从53.89%到58.62%)。

Conclusion: TPA成功整合了时间建模、提示感知对比学习和不确定性量化，为胎儿先天性心脏病超声视频分类提供了一个有效的框架，显著提升了分类性能和临床可靠性。
...
TL;DR: BasketLiDAR是首个结合LiDAR点云和多视角摄像头的篮球运动多模态数据集，提出了基于LiDAR的实时3D多目标跟踪框架，在遮挡条件下仍能实现优越的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于多摄像头的体育运动员跟踪系统受限于2D视频数据和复杂的3D重建处理，难以实现实时分析。篮球运动中球员快速移动、频繁遮挡，使得实时3D轨迹跟踪极具挑战性。

Method: 构建BasketLiDAR多模态数据集（包含4,445帧和3,105个球员ID），提出新颖的MOT框架：包括仅使用LiDAR的实时跟踪管道和融合LiDAR与摄像头数据的多模态跟踪管道，利用LiDAR的高精度3D空间信息。

Result: 实验结果表明，该方法实现了传统纯摄像头方法难以达到的实时操作，即使在遮挡条件下也能获得优越的跟踪性能。

Conclusion: BasketLiDAR数据集和提出的MOT框架为体育领域的实时3D运动员跟踪提供了有效解决方案，结合LiDAR技术解决了传统方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有伪装目标检测方法需要大量训练和计算资源，而基础模型如SAM需要高质量提示才能表现良好，但手动生成提示成本高效率低。

Method: 将COD解耦为两个阶段：RAG阶段通过无监督聚类构建检索数据库生成粗掩码作为提示，SEG阶段使用SAM2进行精细分割。

Result: 在基准COD数据集上的广泛实验表明，RAG-SEG性能与或超过最先进方法，所有实验均在个人笔记本上完成。

Conclusion: RAG-SEG提供了一种无需传统训练的高效实用COD解决方案，计算效率高且性能竞争力强。
...
TL;DR: VideoEraser是一个无需训练即可防止文本到视频扩散模型生成不良内容的即插即用框架，通过两阶段处理实现选择性内容擦除，在多个任务上平均减少46%的不良内容生成。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型的快速发展引发了隐私、版权和安全方面的担忧，这些模型可能被滥用来生成有害或误导性内容，需要一种有效的方法来防止不良内容的生成。

Method: 提出VideoEraser框架，采用两阶段处理：选择性提示嵌入调整（SPEA）和抗干扰噪声引导（ARNG），作为即插即用模块与现有T2V扩散模型无缝集成。

Result: 在对象擦除、艺术风格擦除、名人擦除和露骨内容擦除四个任务上，VideoEraser在效能、完整性、保真度、鲁棒性和泛化性方面均优于现有方法，平均减少46%的不良内容生成。

Conclusion: VideoEraser提供了一种有效的训练无关解决方案，能够显著抑制文本到视频生成中的不良内容，为解决扩散模型的安全和伦理问题提供了实用工具。
...
Method: 使用深度学习模型进行姿态检测，然后结合序列建模技术进行时序预测，构建端到端的深度学习框架

Result: GRU在意图预测准确性方面表现优于LSTM，而1D CNN在预测速度方面表现最佳
...
TL;DR: RCDINO是一个基于transformer的多模态3D目标检测模型，通过融合DINOv2预训练模型的语义特征来增强视觉主干特征，在nuScenes数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 自动驾驶和机器人技术需要有效的多模态数据融合，特别是相机和雷达数据的融合，以提升3D目标检测性能

Method: 提出RCDINO模型，将预训练DINOv2基础模型的语义丰富表示与视觉主干特征融合，增强视觉表示能力，同时保持与基线架构的兼容性

Result: 在nuScenes数据集上达到56.4 NDS和48.1 mAP的state-of-the-art性能，在雷达-相机模型中表现最佳

Conclusion: 通过融合DINOv2的语义特征能有效提升多模态3D目标检测性能，该方法具有架构兼容性和实际应用价值
...
Method: 采用注意力敲除作为主要分析工具，设计三种敲除变体（时间、空间、语言到视频），通过控制敲除层窗口进行全局和细粒度设置的分析。

Result: 发现三个关键结果：1）早期层主要负责视频信息提取；2）某些中间层对视频问答有超常影响；3）时空建模更依赖语言引导检索。这些洞察可用于减少注意力计算。
...
Method: 使用进化算法识别需要微调的层子集，专注于相关层进行微调，减少可训练参数数量。
...
Method: 使用图像条件化的高斯溅射量化器，联合利用高斯间和属性间相关性，采用跨训练场景的共享码本，编码、量化和解码过程联合训练，支持基于解码时图像的场景条件解码。

Result: 实验结果表明ICGS-Quantizer在压缩效率和场景变化适应性方面持续优于最先进方法，将3DGS存储需求降低到千字节范围同时保持视觉保真度。

Conclusion: ICGS-Quantizer有效解决了3DGS长期归档的存储和适应性限制，为大规模3D场景的压缩和更新提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于3D高斯泼溅的方法在处理驾驶场景时，虽然通过解耦动态和静态组件解决了运动模糊问题，但忽视了具有充分几何关系的背景优化，仅通过添加高斯来拟合每个训练视图，导致新视角渲染鲁棒性有限且缺乏准确的几何表示。

Method: 1) 采用区域体素初始化方案，将场景划分为近、中、远区域以增强近距离细节表示；2) 引入可变形神经高斯来建模非刚性动态物体，其参数通过可学习变形网络进行时间调整；3) 使用预训练模型的深度和法线先验对整个框架进行监督，提高几何结构准确性。

Result: 在Waymo和KITTI数据集上的严格评估表明，该方法在驾驶场景的新视角合成方面实现了最先进的性能。

Conclusion: DriveSplat通过创新的动态-静态解耦策略、区域化初始化和几何先验监督，有效解决了驾驶场景3D重建中的挑战，为高质量的新视角合成提供了可靠的解决方案。
...
TL;DR: 本文针对深度学习模型在抽象推理方面的瓶颈，通过Raven渐进矩阵问题作为基准，提出了因果链建模视角，并基于此设计了DIO基线模型。针对DIO模型的局限性，逐步提出了三种改进方法。
...
Result: 实验发现DIO模型的优化目标（最大化互信息变分下界）无法让模型真正掌握预设的人类推理逻辑，主要原因是下界紧致性影响互信息最大化效果，以及互信息无法捕捉因果关系。

Conclusion: 需要克服互信息最大化方法的局限性，通过渐进改进方法来更好地模拟人类抽象推理过程。
...
TL;DR: 提出SpiVG网络，通过脉冲神经网络和变分推理解决视频摘要中的时序依赖和特征融合噪声问题，在多个数据集上表现优异
...
Method: 设计基于脉冲神经网络的关键帧提取器，引入动态聚合图推理器解耦上下文对象一致性和语义视角连贯性，使用变分推理重建模块处理特征融合不确定性
...
Result: 发现掩码比例和补丁大小可以控制MAE学习短程和长程空间相关性特征，且MAE表示能够适应超越二阶统计量的空间相关性。

Conclusion: 研究为实践中如何选择MAE超参数提供了理论见解，帮助更好地理解MAE学习机制。
...
TL;DR: 提出BIRD方法，通过双向时间信息传播同时利用局部相邻帧和全局过去未来帧信息，实现红外小目标检测的SOTA性能和快速推理
...
Method: 设计双向传播策略：前向和后向分支分别包含局部时间运动融合(LTMF)模块建模相邻帧时空依赖，以及全局时间运动融合(GTMF)模块聚合全局传播特征与局部融合特征，最后融合双向特征进行检测，并采用检测损失和时空融合(STF)损失联合优化

Result: 大量实验证明BIRD方法不仅达到最先进的性能，还显示出快速的推理速度

Conclusion: BIRD方法通过双向时间信息传播有效解决了滑动窗口方法的局限性，在红外小目标检测任务中取得了优异性能
...
TL;DR: 基于YOLOv8深度学习框架的汽车微观凹陷自动检测方案，通过自定义数据集和模型优化，实现了高准确度的实时检测能力


<details>
  <summary>Details</summary>
Motivation: 传统汽车损坏检测方法劳动密集、手工操作且容易漏检微观表面缺陷，需要更快速精准的自动化检测方案

Method: 构建自定义汽车表面图片数据集，基于YOLOv8m模型开发YOLOv8m-t4和YOLOv8m-t42两个优化版本，采用实时数据增强技术进行训练

Result: YOLOv8m-t42模型表现最佳：精度0.86、召回0.84、F1分0.85，mAP@0.5为0.60，PR曲线面积0.88，具备实时检测能力

Conclusion: 该方法能够有效检测汽车微观表面缺陷，在自动保险评估和汽车检测等实际应用中具有良好的应用前景
...
TL;DR: 一种新的无SfM的3D高斯抽射方法，能够从极稀疏视图输入中聚合估计相机姿势和重建3D场景，在2张训练图像下达到了2.75dB的PSNR提升。


<details>
  <summary>Details</summary>
Motivation: 3D高斯抽射依赖密集多视图和精确相机姿势，在极稀疏输入时SfM方法失败，导致渲染质量下降。

Method: 提出密集立体模块进行相机姿势估计和全局点云初始化；一致性视图插值模块生成额外监督信号；多尺度拉普拉斯一致性正则化和适应性空间多尺度几何正则化。

Result: 在2张训练图像的极稀疏条件下，PSNR提升2.75dB，综合性能显著超过现有最佳方法，渲染图像变形最小但保留了丰富的高频细节。

Conclusion: 该方法有效解决了极稀疏视图下3DGS的初始化问题，为少视图3D重建提供了可靠的方案，具有重要的实际应用价值。
...
TL;DR: MExECON是一个从稀疏多视角RGB图像进行3D着装人体重建的新方法，通过多视角联合优化提升了几何和姿态估计精度


<details>
  <summary>Details</summary>
Motivation: 现有的单视角方法ECON在重建精度上有限，需要利用多视角信息来提升几何细节和身体姿态估计的准确性

Method: 提出JMBO算法联合优化SMPL-X身体模型，保持多视角一致性；通过法线图积分添加几何细节，整合前后视角的法线信息

Result: 实验结果显示MExECON相比单视角基线方法在保真度上持续提升，与现有少样本3D重建方法相比具有竞争力

Conclusion: MExECON成功扩展了单视角方法到多视角场景，无需重新训练网络即可实现显著的重建质量提升
...
<details>
  <summary>Details</summary>
Motivation: 从驾驶记录中合成外推视图对于自动驾驶模拟至关重要，但现有方法使用生成先验作为伪真值往往导致几何一致性差和渲染过度平滑的问题。
...
Conclusion: ExtraGS通过整合几何和生成先验，有效解决了自动驾驶场景外推视图合成中的关键挑战，为自动驾驶模拟提供了更高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有草图动画方法在处理多对象交互和复杂运动时存在局限，要么仅限于单对象情况，要么存在时间不一致性和泛化能力差的问题，需要一种更有效的解决方案。

Method: 采用两阶段流程：1）运动初始化阶段交互式分割语义组并定义关键帧，通过插值生成粗略动画；2）运动细化阶段使用基于组的位移网络（GDN）预测组特定位移场，并利用文本到视频模型的先验，包含上下文条件特征增强（CCFE）等专门模块来改善时间一致性。

Result: 大量实验表明，该方法在生成高质量、时间一致的多对象复杂草图动画方面显著优于现有方法。

Conclusion: GroupSketch方法成功扩展了草图动画的实际应用范围，为复杂多对象交互场景提供了有效的解决方案。
...
TL;DR: D3FNet是一个针对高分辨率遥感影像中狭窄道路提取的深度学习网络，通过差分注意力机制和双流解码融合，在复杂场景下实现了精细道路结构分割的优异性能。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率遥感影像中狭窄道路提取的挑战，包括道路宽度有限、拓扑结构碎片化、频繁遮挡等问题，传统模型难以处理细粒度、遮挡和低对比度的道路段。
...
TL;DR: ADAPT是一种无需反向传播的测试时自适应方法，通过高斯概率推理建模类别条件似然，利用CLIP先验和历史知识库进行轻量级正则化，在各种分布偏移下实现最先进性能。
...
Method: 将TTA重新构建为高斯概率推理任务，使用逐步更新的类别均值和共享协方差矩阵建模类别条件似然。引入基于CLIP先验和历史知识库的轻量级正则化来纠正潜在似然偏差。无需源数据、梯度更新或完整访问目标数据。

Result: 在多样化基准测试上的广泛实验表明，该方法在各种分布偏移下实现了最先进的性能，具有优越的可扩展性和鲁棒性。

Conclusion: ADAPT提供了一种高效、可扩展的测试时自适应解决方案，通过概率推理框架和轻量级正则化机制，解决了现有方法的局限性，为实时部署提供了可行路径。
...
TL;DR: 提出一种两阶段训练策略，通过邻域感知软掩码自适应地为局部变化强烈的像素分配更高权重，引导INR模型优先学习高频细节，有效缓解频谱偏差问题。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示(INRs)存在频谱偏差问题，倾向于学习低频分量而难以捕捉高频细节如锐利边缘和精细纹理。现有方法主要通过架构修改或特殊激活函数来解决，本文探索通过直接指导训练过程的新方向。

Method: 采用两阶段训练策略：1)使用邻域感知软掩码自适应地为局部变化强烈的像素分配更高权重，鼓励模型早期关注精细细节；2)过渡到全图像训练。

Result: 实验结果表明该方法能持续提升重建质量，并与现有INR方法形成互补。

Conclusion: 作为首个在图像INR中为像素分配频率感知重要性的尝试，本工作为解决频谱偏差问题提供了新途径。
...
TL;DR: 这篇论文提出了一种新的线性时间凸杰松弛方法和合同器方法，大大加快了分支界定法在点云注册中的运行速度，能在半秒内求解100个点的全局最优解，比现有SDP法快两个数量级。


<details>
  <summary>Details</summary>
Motivation: 虽然截断最小二乘(TLS)能够处理达95%的离群点，但求解这个组合优化问题的全局最优解极其困难，现有的半正定规划(SDP)方法需要数百秒时间。

Method: 提出了新的线性时间凸杰松弛方法，以及一种合同器方法来加快分支界定法(BnB)的运行速度。

Result: 在提供旋转轴的情况下，新方法能在半秒内完成100个点的3D点云注册，比当前最先进的SDP法STRIDE快两个数量级。

Conclusion: 该方法不仅提供了全局最优性的形式证明，还通过对抗性实例进行了实证验证，显示出在近全局最优解的局部极小值情况下仍能找到全局最优解。
...
Result: YOLOv11s模型表现最佳（精度96.2%，召回率92.3%，mAP50为96.7%），无人机热成像提供了补充的空中视角，空间分析识别出明确的活动热点区和潜在冲突区域。

Conclusion: 多视角监测与自动化目标检测相结合，能够增强野生动物监测和景观管理能力，有效揭示保护景观内的人兽冲突情况。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频LLM在时序感知方面存在不足：时间戳编码不明确、帧级特征连续性弱、语言视觉对齐漂移，需要更精细的时间推理和实体定位能力。

Method: 1. 扩散时序潜在编码器增强边界敏感性和时序一致性；2. 对象接地表示将查询实体显式绑定到局部视觉证据；3. 混合令牌方案使用离散时间令牌进行显式时间戳建模。

Result: 在Charades STA、NExT GQA和多个VideoQA基准测试中取得了最先进的性能，验证了强大的接地能力。

Conclusion: Grounded VideoDiT通过三项关键创新有效解决了视频时序感知的挑战，为视频理解提供了更精细的时间推理和实体交互分析能力。
...
TL;DR: 提出了一种弱监督方法，通过人工质量评级来训练评分模型，进而优化ALS数据的树木实例分割，在正确识别树木实例方面提升了34%的性能。
...
Method: 使用非微调模型或闭式算法获得初始分割结果，由人工操作员进行质量评级，训练评分模型分类分割输出，最后用评分模型的反馈微调分割模型。

Result: 原始分割模型在正确识别树木实例方面提升了34%，同时显著减少了非树木实例的预测数量。

Conclusion: 该方法有效提升了树木分割性能，但在稀疏林区小树木和复杂环境中的表现仍有挑战。
...
TL;DR: 提出CFG方法，通过关键特征引导提高3D点云对抗样本的迁移性，在黑盒攻击场景下无需目标模型信息即可生成有效对抗样本
...
Conclusion: CFG方法通过关键特征引导有效提高了3D点云对抗样本的迁移性，在黑盒攻击场景下表现出色
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖离线地图先验和多模态传感器，导致推理时计算开销大且需要陈旧地图数据，需要开发更高效、低成本的视觉中心方案

Method: 采用教师-教练-学生三级知识蒸馏框架，包含多模态教师模型、视觉教练模型和轻量学生模型，并提出了TGPD和MSRD两种蒸馏策略

Result: 在nuScenes数据集上，学生模型性能提升+6.68 mIoU和+10.94 mAP，同时推理速度加快

Conclusion: MapKD框架成功实现了从多模态到单模态的知识转移，为自动驾驶系统提供了高效、低成本的在线高清地图构建解决方案
...
TL;DR: CM2LoD3提出了一种基于冲突图(CMs)的自动化LoD3建筑模型重建方法，通过语义分割真实和合成的冲突图来重建包含窗户、门等立面细节的精细建筑模型。


<details>
  <summary>Details</summary>
Motivation: 现有LoD1和LoD2建筑模型缺乏立面细节，而传统LoD3模型需要手动建模，难以大规模应用。需要自动化方法来重建包含窗户、门等立面元素的详细建筑模型。

Method: 利用射线到模型先验分析获得冲突图(CMs)，开发语义冲突图生成器(SCMG)生成合成CMs，通过语义分割真实和合成CMs，并结合纹理模型分割结果进行置信度融合。
...
Method: 提出多维动态提示路由(MDPR)框架，构建涵盖五个视觉语义维度的类别知识库，通过动态路由机制对齐全局视觉类别、检索最优提示并平衡细粒度语义，通过logits融合获得稳定预测
...
Method: 提出StreamMem机制，以流式方式编码新视频帧，使用视觉token与通用查询token之间的注意力分数压缩KV缓存，并维持固定大小的KV内存
...
TL;DR: 提出了分子赋能的All-in-SAM模型，通过分子赋能学习、SAM适配器和分子导向校正学习，显著提升细胞分类性能，减少标注工作量。
...
Method: 采用全栈方法：1）分子赋能学习减少像素级标注需求；2）SAM适配器增强特定语义识别；3）分子导向校正学习（MOCL）提升分割精度。
...
TL;DR: Waver是一个高性能的多模态生成基础模型，能够统一进行文本到视频(T2V)、图像到视频(I2V)和文本到图像(T2I)生成，支持5-10秒720p原生分辨率视频生成，并在多项基准测试中排名前三。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视频生成模型在模态对齐、训练效率和生成质量方面的挑战，开发一个统一的框架来同时支持图像和视频生成任务。

Method: 采用混合流DiT架构增强模态对齐和加速训练收敛；建立全面的数据筛选流程，使用MLLM视频质量模型手动标注和筛选高质量训练样本；提供详细的训练和推理方案。

Result: Waver能够生成复杂运动的高质量视频，在运动幅度和时间一致性方面表现优异，在Artificial Analysis的T2V和I2V排行榜中均排名前三，超越现有开源模型并媲美商业解决方案。

Conclusion: Waver作为一个统一的多模态生成模型，在视频生成质量和技术性能方面达到了先进水平，为社区提供了高效训练高质量视频生成模型的技术方案，有望加速视频生成技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型在捕捉多样化姿势和形状的细节变化方面存在局限，主要由于训练数据多样性不足和建模假设限制，且内部骨骼与外部软组织之间存在问题依赖关系。

Method: 使用240个同步相机捕获的60万高分辨率扫描数据，明确解耦形状和骨骼基础，将网格表示基于人体骨骼，实现非线性姿态校正。

Result: ATLAS在拟合未见过的多样化姿势主体时更准确，定量评估显示非线性姿态校正比线性模型更有效地捕捉复杂姿势。

Conclusion: 通过骨骼基础的解耦方法，ATLAS实现了增强的形状表达能力、细粒度身体属性定制以及独立于外部软组织特征的关键点拟合，显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在指令引导图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠的问题，会产生意外的伪修改和编辑指令遵循度不足。自回归模型提供了因果和组合机制，能够自然规避扩散方法的这些问题。

Method: 提出VAREdit框架，将图像编辑重新定义为下一尺度预测问题。通过源图像特征和文本指令的条件，生成多尺度目标特征。引入尺度对齐参考(SAR)模块，在第一个自注意力层注入尺度匹配的条件信息，解决最精细尺度源特征无法有效指导粗粒度目标特征预测的问题。
...
<details>
  <summary>Details</summary>
Motivation: 在真实应用中，用户通常需要获得一组多样化的输出（如图像），而传统的独立采样方法往往产生冗余结果，限制了用户选择和创意探索。

Method: 将组推理建模为二次整数分配问题：候选输出作为图节点，通过优化样本质量（一元项）和最大化组多样性（二元项）来选择子集；采用渐进式剪枝策略提高运行效率。

Result: 实验表明，该方法在文本到图像、图像到图像、图像提示和视频生成等多种任务中，相比独立采样基线和最新推理算法，显著提升了组多样性和质量。
...
TL;DR: CineScale是一种无需训练即可实现更高分辨率视觉生成的新型推理范式，能够实现8K图像生成和4K视频生成
...
Method: 提出CineScale推理范式，针对不同类型的视频生成架构设计专用变体，无需微调即可实现高分辨率图像和视频生成

Result: 实验验证了该范式在扩展高分辨率视觉生成能力方面的优越性，实现了8K图像生成和4K视频生成

Conclusion: CineScale扩展了高分辨率视觉生成的范围，包括I2V和V2V合成，为图像和视频模型的高分辨率生成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决手术部位感染(SSI)预防中的数据隐私、标注成本高和缺乏公开数据集的问题，填补手术伤口筛查工具的空白

Method: 1) 构建包含697张手术伤口图像的SurgWound数据集，由3位专业外科医生标注8个临床属性；2) 建立包含视觉问答和报告生成任务的基准测试；3) 提出三阶段学习框架WoundQwen：第一阶段用5个MLLM预测伤口特征，第二阶段用2个MLLM进行诊断，第三阶段整合结果生成综合报告

Result: 开发了首个开源手术伤口数据集和基准测试，提出了能够分析伤口特征并提供个性化护理指导的三阶段诊断框架
